...
const inpcount = document.querySelector(`.inpcount`);                
...
document.onclick = (event) => {
  ...
  if (event.target.classList.contains(`divx`) && cardPosition !== null) {
    cardStoreg.splice(cardPosition, 1);

    localStorage.setItem("cart", JSON.stringify(cardStoreg));
    renderCart();
    location.reload();
  }
};
